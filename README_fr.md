...
> Veille techno quotidienne et automatisée — IA/ML, outillage LLM, RAG & agents, MLOps, DevOps, cloud, infra et outils de dev.
...
- OpenAI a présenté la série GPT‑5.6 : Sol (modèle phare), Terra (2× moins cher que GPT‑5.5 avec des performances compétitives) et Luna (le moins cher), avec des tarifs par million de tokens à 5 $/30 $ (Sol), 2,50 $/15 $ (Terra) et 1 $/6 $ (Luna) [1, 2].
- Le gouvernement américain a demandé un déploiement échelonné de GPT‑5.6, initialement auprès d'environ 20 partenaires de confiance via Amazon Bedrock, signalant un contrôle réglementaire sur les modèles frontaliers [3, 4, 5].
- Liquid AI a publié LFM 2.5 (230 millions de paramètres), un modèle non-transformer utilisant des réseaux de neurones à espace d'état/liquides, qui rivalise avec des modèles transformer 3× plus grands sur des tâches de raisonnement séquentiel en périphérie [6].
- DeepReinforce a publié Ornith‑1.0, une famille de modèles de codage open-source basée sur Gemma 4/Qwen 3.5, à la pointe pour des tailles comparables [7].

## Adoption de l'IA, outils et agents
- OpenAI signale une adoption interne explosive de Codex : le nombre médian de tokens générés a augmenté de 56× (Recherche), 32× (Support), 27× (Ingénierie) et 13× (Juridique) depuis novembre 2025 [8].
- Un partenaire Deloitte en IA utilise quotidiennement Anthropic/OpenAI/Google pour des newsletters, la surveillance du marché et des workflows de podcasts [9].
- GitHub Copilot ajoute MAI‑Code‑1‑Flash (modèle de codage interne de Microsoft) pour les formules Business/Enterprise avec une facturation à l'usage et une faible latence ; les administrateurs doivent activer la stratégie [10].
- Vercel AI SDK 7 ajoute le streaming sans surcharge et l'orchestration d'outils avec une télémétrie unifiée pour l'utilisation des tokens, les choix de modèles et la latence des outils [11].
- Cloudflare intègre des harnais d'agents (par exemple, Flue) à sa plateforme avec exécution durable, code en bac à sable et primitives d'état persistantes [12].
- Orca (open-source) exécute plusieurs agents de codage (Claude Code, Codex, OpenCode) côte à côte dans des environnements isolés pour comparaison et gestion [13].
- GitHub Desktop 3.6 intègre Copilot pour la rédaction de commits et la résolution de conflits de fusion, et ajoute la prise en charge des *worktrees* Git [14].

## Sécurité, gouvernance et économie de l'IA
- Un tribunal allemand déclare Google responsable des erreurs dans AI Overview, renforçant la responsabilité des déployeurs pour les inexactitudes de l'IA [15, 16].
- Un assistant basé sur OpenClaw (Fiu) a résisté à un test de stress de 6 000 e-mails menés par 2 000 attaquants sans fuite de secrets ni réponses non autorisées, démontrant une résilience face à l'injection de prompts et à l'ingénierie sociale [17, 18].
- Les coûts élevés des LLM restent insoutenables pour de nombreuses entreprises ; les tarifs des modèles frontaliers et les gains stagnants exercent une pression sur les acteurs établis alors que les alternatives open-weight et locales se développent [19, 20].
- Kaspersky signale 33 352 attaques (janvier-avril) se faisant passer pour des services d'IA (par exemple, Claude, OpenClaw), soit près de 5× plus qu'en 2023, les faux outils d'IA dépassant désormais les leurres de logiciels bureautiques contrefaits [21].

## MLOps, DevOps et infrastructure
- Hugging Face permet le déploiement d'un serveur vLLM privé en une seule commande via HF Jobs avec une facturation à la seconde et sans nécessiter Kubernetes [22].
- Argo CD v3.5 RC ajoute une UI ApplicationSet avec aperçus, une version bêta de l'usurpation d'identité/Source Hydrator, la prise en charge de Helm 4, le mTLS pour repo-server et des fonctionnalités GitOps multi-locataires étendues [23].
- Le plugin Kubernetes Headlamp pour Cluster API (CAPI) fournit une UI visuelle pour simplifier le provisionnement/débogage/gestion du cycle de vie des clusters [24].
- Grafana 13.1 GA étend Git Sync (GitLab, BitBucket, commits signés) et Grafana Assistant à huit autres sources de données (Snowflake, Oracle, Elasticsearch) [25].
- Jaeger v2.18 ajoute un backend de stockage ClickHouse avec une compression 8,6× sur 10 millions de spans et un débit >50 000 spans/sec [26].
- AWS DevOps Agent (GA) + Datadog MCP Server permettent une résolution autonome des incidents avec une observabilité corrélée et une télémétrie du code [27].
- GitHub ajoute totals_by_ai_adoption_phase aux rapports Copilot, exposant le nombre total de fusions de PR par phase pour l'analyse du débit et de l'impact [28, 29].
- Modelplane, basé sur Crossplane, unifie des clusters GPU dispersés en une seule flotte d'inférence sans opérateurs personnalisés [30].

## Outils et pratiques de développement
- GitHub Desktop 3.6 approfondit l'intégration de Copilot pour les commits et les conflits de fusion, et prend en charge les *worktrees* Git [14].
- Drizzle ORM a atteint les limites de taille de packument NPM (100 Mo), bloquant les publications pendant un mois ; résolu en supprimant les anciennes versions avec le support de NPM [31].
- no‑mistakes (open-source) agit comme un proxy Git local qui valide les pushes via une IA dans un *worktree* jetable, ouvrant automatiquement des PR avec les corrections [32].
- Runlayer (nouveau) fournit une couche de contrôle d'entreprise pour les agents IA : surveillance des accès, application des identités/autorisations et blocage de l'injection de prompts, de l'exfiltration de données et des agents non gérés [33].

## Sécurité et notes sur les incidents
- Un nouveau malware macOS "GasLight" intègre de fausses erreurs système pour tromper les outils d'analyse IA ; il fonctionne comme un voleur d'informations/backdoor [34].
- L'extension Edge malveillante "Edgecution" abuse de Chrome Native Messaging pour se connecter à une backdoor Python, distribuée via de faux portails Microsoft [35].
- Une attaque sur le frontend de Polymarket via un fournisseur compromis a vidé ~3 M$ de 11 portefeuilles ; les fonds ont été échangés contre de l'ETH et envoyés à une seule adresse ; des remboursements complets ont été promis [36].
